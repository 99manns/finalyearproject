...
abstract class User implements ActiveRecordInterface
{
    ...
    public function setPermissionid($v)
    {
        ...
        if ($this->permissionid !== $v) {
            $this->permissionid = $v;
            $this->modifiedColumns[UserTableMap::COL_PERMISSIONID] = true;
        }

        if ($this->aPermission !== null && $this->aPermission->getPermissionid() !== $v) {
    ...
    public function setDelted($v)
    {
        ...
        if ($this->delted !== $v) {
            $this->delted = $v;
            $this->modifiedColumns[UserTableMap::COL_DELTED] = true;
        }
    ...
    public function setGenderid($v)
    {
        ...
        if ($this->genderid !== $v) {
            $this->genderid = $v;
            $this->modifiedColumns[UserTableMap::COL_GENDERID] = true;
        }

        if ($this->aGender !== null && $this->aGender->getGenderid() !== $v) {
            $this->aGender = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserTableMap::translateFieldName('Permissionid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->permissionid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserTableMap::translateFieldName('Delted', TableMap::TYPE_PHPNAME, $indexType)];
            $this->delted = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('Genderid', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    {
        if ($this->aPermission !== null && $this->permissionid !== $this->aPermission->getPermissionid()) {
            $this->aPermission = null;
        }
        if ($this->aGender !== null && $this->genderid !== $this->aGender->getGenderid()) {
            $this->aGender = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aGender !== null) {
                if ($this->aGender->isModified() || $this->aGender->isNew()) {
                    $affectedRows += $this->aGender->save($con);
                }
                $this->setGender($this->aGender);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_PERMISSIONID)) {
            $modifiedColumns[':p' . $index++]  = 'PermissionID';
        }
        if ($this->isColumnModified(UserTableMap::COL_DELTED)) {
            $modifiedColumns[':p' . $index++]  = 'delted';
        }
        if ($this->isColumnModified(UserTableMap::COL_GENDERID)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setPermissionid($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setDelted($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setGenderid($arr[$keys[6]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_PERMISSIONID)) {
            $criteria->add(UserTableMap::COL_PERMISSIONID, $this->permissionid);
        }
        if ($this->isColumnModified(UserTableMap::COL_DELTED)) {
            $criteria->add(UserTableMap::COL_DELTED, $this->delted);
        }
        if ($this->isColumnModified(UserTableMap::COL_GENDERID)) {
            $criteria->add(UserTableMap::COL_GENDERID, $this->genderid);
        }
    ...
    /**
     * Declares an association between this object and a ChildGender object.
     *
     * @param  ChildGender $v
     * @return $this|\User The current object (for fluent API support)
     * @throws PropelException
     */
    public function setGender(ChildGender $v = null)
    {
        if ($v === null) {
            $this->setGenderid(NULL);
        } else {
            $this->setGenderid($v->getGenderid());
        }

        $this->aGender = $v;

        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildGender object, it will not be re-added.
        if ($v !== null) {
            $v->addUser($this);
        }
    ...
    /**
     * Get the associated ChildGender object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildGender The associated ChildGender object.
     * @throws PropelException
     */
    public function getGender(ConnectionInterface $con = null)
    {
        if ($this->aGender === null && ($this->genderid !== null)) {
            $this->aGender = ChildGenderQuery::create()->findPk($this->genderid, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aGender->addUsers($this);
             */
        }
    ...
    {
        if ($this->aPermission === null && ($this->permissionid !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPermission->addUsers($this);
             */
        }
    ...
    {
        if (null !== $this->aGender) {
            $this->aGender->removeUser($this);
        }
        if (null !== $this->aPermission) {
            $this->aPermission->removeUser($this);
        }
        $this->userid = null;
        $this->firstname = null;
        $this->lastname = null;
